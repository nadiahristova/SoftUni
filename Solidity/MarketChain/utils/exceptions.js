...
async function tryCatch(promise, message) {
    try {
        ...
        //console.log(error)
        let errMsg = error.message.replace("Returned error:", "").trim()
        assert(errMsg.startsWith(PREFIX + message), "Expected an error starting with '" + PREFIX + message + "' but got '" + error.message + "' instead");
    }
};
